...
public class FilmorateVariableStorageDaoImpl<TE, T> extends FilmorateConstantStorageDaoImpl<TE>
        implements FilmorateVariableStorageDao<TE, T> {
    ...
    @Override
    public TE deleteById(long objectId, long anotherObjectId) throws ObjectNotFoundInStorageException {
        TE objectEntity;
    ...
    @Override
    public TE save(T object) {                    // Из-за того, что надо получать данные из объекта непосредственно,
        return null;      // Будем переопределять метод save для всех классов data-слоя с изменяющимися хранилищами
    }

    @Override
    public TE update(T object) {   // А метод update не будем переопределять для классов LikeEntity и FilmGenreEntity
        return null;
